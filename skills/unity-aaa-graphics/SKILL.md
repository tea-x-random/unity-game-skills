...
The per-surface source choice is **PixelLab vs Gemini vs Tripo**, not just generate-vs-procedural. **Decision rule: if the requested finish is PIXEL ART → `unity-pixel-art` / PixelLab for final sprites, tiles, icons, sheets, and rotations.** Gemini may explore concepts/style boards, but final pixel assets must be pixel-native. **If the surface is non-pixel and MOVES or needs multiple angles → Tripo** (rig + animate; for high-res/painterly 2D, render cycles to sprite frames). If it is static non-pixel art (tiling grounds, backgrounds, UI, icons, single-angle props) → Gemini. Never make pixel art by Tripo/3D downscale.
...
- **`unity-pixel-art`** — final pixel-native sprites, tilesets, icons, directional sheets, and animation strips.
- **`unity-image-generator`** — Gemini concepts, static non-pixel sprites, environment textures/tilesets, UI; the AAA prompt library lives alongside it and here.
- **`unity-3d-generator`** — 3D models/props and non-pixel pre-rendered 2D.
