...
**Why:** Unity must compile scripts before they're usable. `create_script` and `script_apply_edits` already trigger import and compilation automatically — calling `refresh_unity` afterward is redundant. **Do not use a freshly-added type before `is_compiling == false` and the console is clean** — it silently fails or throws. If you edited via plain file write (not `create_script`/`script_apply_edits`), kick the compile yourself: `refresh_unity(mode="force", scope="scripts", compile="request", wait_for_ready=True)`, then poll.

### 1a. `execute_code` Is a C# 6 Method Body (CodeDom), Not a Sandbox

`execute_code` compiles a **C# 6 method body via CodeDom** when `compiler="auto"` (the default) and Roslyn (Microsoft.CodeAnalysis) is not installed. It is real C# — `System.DateTime.Now` and the like work — but the CodeDom path means:

- **No `using` directives, no local functions, no C#7+ syntax.** Use **fully-qualified type names** (e.g. `UnityEditor.EditorApplication.isCompiling`).
- Build multi-line output with `System.Text.StringBuilder` and `return` a value to read it back.
- Pass `compiler="roslyn"` only when Roslyn is actually present.
...
**Portrait iPhone framing of Screen-Space-Overlay UI.** Screenshots capture at the Game view's current size; there is no MCP param to force portrait. Add and select a portrait FixedResolution size via `execute_code` + reflection (these `UnityEditor` types are internal):
1. `GameViewSizes` ScriptableSingleton → `.currentGroup` → `AddCustomSize(new GameViewSize(GameViewSizeType.FixedResolution, w, h, label))`.
2. On the GameView window call the private `SizeSelectionCallback(index, null)`, then `Repaint()`.
3. `manage_camera(action="screenshot")` with **no `camera` arg** (ScreenCapture API path) now grabs overlay UI at that resolution.

Common portrait sizes: **1170x2532** (iPhone 12/13/14), **1080x1920**.
...
- Documented `execute_code` = CodeDom C# 6 method body (no `using`/local-functions/C#7+, fully-qualified names, StringBuilder+return); strengthened post-edit compile gate; added portrait iPhone screenshot recipe via `GameViewSizes` reflection.
