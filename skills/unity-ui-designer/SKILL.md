...
1. **Pick a font that covers the required scripts.** The example game needed kanji (牛, 子牛, 雄牛, 和牛); LiberationSans has none → it switched to **Zen Maru Gothic** (OFL-licensed, full JP coverage). Verify coverage before committing — a font that can't render the script just yields tofu.
2. **Build a dynamic-atlas `TMP_FontAsset` for crisp SDF.** Use `AtlasPopulationMode.Dynamic` so arbitrary glyphs (any kanji a locale needs) are added at runtime, with **multi-atlas fallback** when one atlas fills ([Unity Learn](https://learn.unity.com/tutorial/textmesh-pro-font-asset-creation-1)). Concrete crispness settings that worked for the example game (regenerated as "ZenMaruGothic SDF HD"): **samplingPointSize 110, atlas 2048², padding 11, Render Mode SDFAA**. Unity's general guidance: SDF render mode, sampling 50–70 for plain Latin, **cap atlas at 2048² for mobile**, padding ~1:10 of sampling ([Unity Learn](https://learn.unity.com/tutorial/textmesh-pro-font-asset-creation-1)) — the example game went higher on sampling for large display type to stay crisp. *Blurry text = sampling/atlas too low for the display size.*
...
5. **No tofu check.** After wiring, screenshot a screen showing the required script and confirm real glyphs, not boxes. The example game had to regenerate the TMP asset multiple times for crispness *and* re-verify kanji rendered — do it once, deliberately, up front.
