...
- **2D sprite animation** — for 2D/top-down/side games, UI, and any asset authored as sprites. Frame-by-frame strips driven by Unity Animation Clips.
- **3D skeletal animation** — for 3D/2.5D assets: Tripo auto-rig + generated animation cycles, imported as a humanoid/generic rig and driven by an Animator.
...
Pick per asset: a bobbing coin needs only a tween; an archer needs authored idle + aim + fire with a frame event.
...
The highest-quality, drift-free way to get 2D animation frames is to **rig + animate a 3D model in Tripo and render each frame to a sprite strip** (`../unity-3d-generator/references/prerender-2d.md`) — one rigged model holds identity across every frame, whereas independently generating each frame with an image model causes identity drift. Generating frame strips directly with Gemini (below) is the lighter-weight alternative when a pre-render is overkill.
